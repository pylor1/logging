...
After an introductory discussion setting the scene, what is logging, what is -not- logging, and some chat, we expand on the terms and definitions in the scope and try to form a set of parameters for defining what we want from logging and how we might go about getting it.
